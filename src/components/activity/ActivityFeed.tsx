  ...
  return (
    <div className={`flex gap-5 group cursor-pointer hover:bg-accent/20 transition-colors rounded-lg border border-border/40 border-l-[3px] ${pcfg.borderAccent} ${pcfg.bg} ${isDimmed ? "opacity-60 hover:opacity-80" : ""} px-6 py-5`}>
      <div className="flex flex-col items-center shrink-0 w-14 pt-0.5">
        ...
        <span className="text-xs font-mono text-muted-foreground/30 mt-1 leading-none whitespace-nowrap">{event.timeAgo}</span>
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2.5 mb-1.5">
          ...
          <span className={`text-xs font-mono uppercase px-2 py-1 rounded border shrink-0 ${ccfg.color}`}>{ccfg.label}</span>
        </div>
        <p className="text-sm text-foreground/50 leading-relaxed line-clamp-2">{event.description}</p>
        <div className="flex items-center gap-2 mt-2">
          <span className="text-xs font-mono text-muted-foreground/50">via {event.source}</span>
        </div>
      </div>
        ...
        {blocks.map((block) => (
          <div key={block.label}>
            <div className="flex items-center gap-2.5 mb-3">
              <span className="text-xs font-mono uppercase tracking-widest text-muted-foreground/50">{block.label}</span>
