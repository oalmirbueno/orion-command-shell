...
const MOCK_ITEMS: AttentionItem[] = [
  {
    id: "1",
    priority: "critical",
    title: "Pipeline de ingestão com latência elevada",
    context: "Data Pipeline · P95 > 200ms há 12min",
    timestamp: "12min",
  },
  {
    id: "2",
    priority: "warning",
    title: "Agente Classifier atingiu 85% de memória",
    context: "ML Processor · Cluster East",
    timestamp: "28min",
  },
  {
    id: "3",
    priority: "warning",
    title: "3 tarefas na fila há mais de 5 minutos",
    context: "Queue Manager · Threshold: 3min",
    timestamp: "6min",
  },
  {
    id: "4",
    priority: "info",
    title: "Deploy v2.14.3 aguardando aprovação",
    context: "Release Pipeline · Staging validated",
    timestamp: "1h",
  },
];
    ...
    <section>
      <div className="flex items-center gap-2 mb-3">
        ...
        <div className="flex items-center gap-1.5 ml-2 px-2 py-0.5 rounded-full bg-status-warning/10 border border-status-warning/20">
          <span className="text-[9px] font-mono text-status-warning font-medium">{MOCK_ITEMS.length}</span>
        </div>
      ...
      <div className="space-y-1.5">
        {MOCK_ITEMS.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className={`flex items-center gap-3 px-4 py-3 rounded-md border border-border/40 ${config.bg} border-l-2 ${config.border} cursor-pointer hover:bg-accent/50 transition-colors group`}
            >
