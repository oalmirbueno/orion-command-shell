...
/**
 * LiveOperations — real-time view of what's happening right now.
 * Replaces the old ActiveSessions with a more mission-control feel:
 * compact rows with visual urgency, agent attribution, and progress.
 */
        ...
        {OPS.map(op => {
          ...
          return (
            <div
              key={op.id}
              className={`
                flex items-center gap-3 px-4 py-3 rounded-lg border cursor-pointer group transition-colors
                ${isPaused
                  ? "border-border/30 opacity-55 hover:opacity-75"
                  : isHigh
            ...
            >
              ...
              <div className="shrink-0">
                {isPaused
                  ? <Pause className="h-3.5 w-3.5 text-status-warning" />
              ...
              <div className="flex items-center gap-2 shrink-0 w-28">
                <div className="flex-1 h-1.5 bg-surface-3 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all ${isPaused ? "bg-status-warning/50" : "bg-primary"}`}
