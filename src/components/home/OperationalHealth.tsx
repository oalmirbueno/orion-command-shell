  ...
  return (
    <section>
      <div className="flex items-center gap-2 mb-3">
        <h2 className="text-[10px] font-mono uppercase tracking-[0.15em] text-muted-foreground">Saúde Operacional</h2>
        {data && (
      ...
      <OrionDataWrapper state={state} source={source} lastUpdated={lastUpdated} onRetry={refetch} compact>
        <div className="rounded-lg border border-border/50 overflow-hidden">
          {(data || []).map((svc, i) => (
            <div key={svc.name} className={`flex items-center gap-3 px-4 py-2.5 ${i > 0 ? "border-t border-border/30" : ""} hover:bg-accent/30 transition-colors`}>
