  ...
  if (result === "failure") {
    return (
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-xs font-mono text-status-critical">FAIL</span>
      </div>
    );
  }
      ...
      <div className="px-6 py-5">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <code className="text-xs font-mono px-2 py-1 rounded bg-surface-2 border border-border/40 text-muted-foreground/60 shrink-0">
              {job.schedule}
            </code>
          ...
          <div className="flex items-center gap-3 shrink-0 ml-4">
            {job.enabled ? (
              <ToggleRight className="h-5 w-5 text-status-online" />
            ) : (
        ...
        {job.error && (
          <div className={`flex items-start gap-3 mb-4 ml-8 px-4 py-3 rounded-lg border ${hasFailed ? "bg-status-critical/5 border-status-critical/15" : "bg-status-warning/5 border-status-warning/15"}`}>
            ...
            <p className={`text-sm font-mono leading-relaxed ${hasFailed ? "text-status-critical/80" : "text-status-warning/80"}`}>
              {job.error}
            </p>
          </div>
        )}
    ...
    <section>
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="flex items-center gap-2 ml-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/20">
          <span className="text-xs font-mono text-primary font-medium">{MOCK_JOBS.filter(j => j.enabled).length} habilitados</span>
        </div>
