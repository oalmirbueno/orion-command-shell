      ...
      <div className="px-6 py-5">
          ...
          <div className="flex items-center gap-4 mb-4 ml-[52px] flex-wrap">
            {agent.dependsOn.length > 0 && (
              <div className="flex items-center gap-2">
                ...
                {agent.dependsOn.map(dep => (
                  <span key={dep} className="text-[10px] font-mono px-2 py-0.5 rounded bg-surface-2 border border-border/30 text-foreground/50">{dep}</span>
                ))}
              </div>
            )}
            {agent.feeds.length > 0 && (
              <div className="flex items-center gap-2">
                ...
                {agent.feeds.map(f => (
                  <span key={f} className="text-[10px] font-mono px-2 py-0.5 rounded bg-surface-2 border border-border/30 text-foreground/50">{f}</span>
                ))}
              </div>
            )}
          </div>
        ...
        <div className="flex items-center gap-4 pt-3 border-t border-border/15 ml-[52px] flex-wrap">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] font-mono text-muted-foreground/35">Sessões</span>
            <span className={`text-xs font-mono font-medium ${agent.sessions > 0 ? "text-foreground" : "text-muted-foreground/25"}`}>{agent.sessions}</span>
          </div>
          <div className="w-px h-3 bg-border/15" />
          <div className="flex items-center gap-1.5">
            <span className="text-[10px] font-mono text-muted-foreground/35">Tokens</span>
            <span className="text-xs font-mono text-foreground/70">{agent.tokensToday}</span>
          </div>
          <div className="w-px h-3 bg-border/15" />
          <div className="flex items-center gap-1.5">
            <span className="text-[10px] font-mono text-muted-foreground/35">Disponib.</span>
            <span className={`text-xs font-mono ${parseFloat(agent.availability) < 99 ? "text-status-warning" : "text-foreground/70"}`}>{agent.availability}</span>
          </div>
          {agent.status !== "offline" && agent.load > 0 && (
            <>
              <div className="w-px h-3 bg-border/15" />
              <div className="flex items-center gap-2 flex-1 max-w-[120px]">
                <span className="text-[10px] font-mono text-muted-foreground/35">Carga</span>
                <div className="flex-1 h-1.5 bg-surface-3 rounded-full overflow-hidden">
                  <div className={`h-full rounded-full ${loadColor}`} style={{ width: `${agent.load}%` }} />
                </div>
                <span className="text-[10px] font-mono text-muted-foreground/25">{agent.load}%</span>
              </div>
            </>
          )}
        </div>
